...
class RagHelper:
    def __init__(self) -> None:
        ...
        elif eval(app.config.get('USE_GROQ')):
            self.model = ChatGroq(
        ...
        elif eval(app.config.get('USE_GOOGLE_EMBEDDING')):
            self.embeddings = GoogleGenerativeAIEmbeddings(
                model= app.config.get('GEMINI_EMBEDDING_MODEL_ID'), 
                google_api_key= app.config.get('GOOGLE_GENERATIVE_LANGUAGE_API_KEY')
            )
        elif eval(app.config.get('USE_HUGGINGFACE_EMBEDDING')):
