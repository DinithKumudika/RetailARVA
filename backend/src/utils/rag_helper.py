...
class RagHelper:
    def __init__(self) -> None:
        ...
        if eval(app.config.get('USE_OLLAMA')) == True:
            self.model = ChatOllama(
                base_url = app.config.get('OLLAMA_URL'),
                model = app.config.get('OLLAMA_MODEL_ID'),
                temperature = 0.5
            )
        elif eval(app.config.get('USE_GEMINI')) == True:
        ...
        elif eval(app.config.get('USE_GROQ')) == True:
            self.model = ChatGroq(
        ...
        if eval(app.config.get('USE_OLLAMA_EMBEDDING')) == True:
            self.embeddings = OllamaEmbeddings(
                base_url= app.config.get('OLLAMA_URL'), 
                model= app.config.get('OLLAMA_EMBEDDING_MODEL_ID')
            )
        elif eval(app.config.get('USE_GOOGLE_EMBEDDING')) == True:
            self.embeddings = GoogleGenerativeAIEmbeddings(
                model= app.config.get('GEMINI_EMBEDDING_MODEL_ID'), 
                google_api_key= app.config.get('GOOGLE_GENERATIVE_LANGUAGE_API_KEY')
            )
        elif eval(app.config.get('USE_HUGGINGFACE_EMBEDDING')) == True:
