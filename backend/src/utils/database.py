...
def add_reviews_from_csv(csv_file_path: str, session: Session):
    with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
        ...
        for row in csv_reader:
            product_id = int(
                row["Id"]
            )  # Assuming 'id' in CSV corresponds to 'product_id'
            ...
            for review_entry in reviews:
                ...
                if review_match and rating_match:
                    ...
                    review_record = CustomerReview(
                        product_id=product_id, review=review, rating=rating
                    )
...
def add_products_from_csv(csv_file_path: str, session: Session):
    with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
        ...
        for row in csv_reader:
            ...
            side_effects = (
                "NULL"
                if str(row["Potential Side Effects"]).strip().lower() == "none reported"
                else str(row["Potential Side Effects"]).strip().replace("'", "''")
            )
            price = float(str(row["Price"]).replace(",", ""))
            sensitivities = (
                "NULL"
                if str(row["Sensitivities"]).strip().lower() == "none"
                else str(row["Sensitivities"]).strip().replace("'", "''")
            )
            allergens = (
                "NULL"
                if str(row["Allergens"]).strip().lower() == "none"
                else str(row["Allergens"]).strip().replace("'", "''")
            )
